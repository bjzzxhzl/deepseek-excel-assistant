...
const pAutoSet = JSON.parse(await run(`(async()=>{ return JSON.stringify(await execTool('set_values',{sheet:'Sheet1',address:'A1',values:[[9]]})); })()`));
check('自动批准下写入不弹窗', pAutoSet.written === true && els.modal._display !== 'flex', JSON.stringify(pAutoSet));
const pAutoFmt = JSON.parse(await run(`(async()=>{ return JSON.stringify(await execTool('format_range',{sheet:'Sheet1',address:'A1:B2',fillColor:'#FF0000'})); })()`));
check('自动批准下设置格式不弹窗', pAutoFmt.formatted === true && els.modal._display !== 'flex', JSON.stringify(pAutoFmt));
